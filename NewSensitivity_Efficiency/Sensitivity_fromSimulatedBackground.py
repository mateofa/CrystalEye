...
f = open('../NewSensitivity/EffArea.dat', 'r')
lines=f.readlines()
eff=[]
eff_energy=[]
for x in lines:
    print float(x.split(' ')[0])," ",float(x.split(' ')[3])/A_geo
    eff.append(float(x.split(' ')[3])/A_geo)
...
f = open('../NewSensitivity/SensitivityCrystalEye.txt', 'r')
lines=f.readlines()
Sens_old=[]
ener_old=[]
for x in lines:
...
def Sens(B,deltaE,e,deltaOmega):
    return  3/(epsilon_68*e) * np.sqrt(B*deltaOmega/(A_geo*T*deltaE))
...
## IBIS_ISG
f_IBIS_ISG = open('./IBIS_ISGRI_erg_Mev.dat', 'r')
...
## IBIS_PIC
f_IBIS_PIC = open('./IBIS_PICs_erg_Mev.dat', 'r')
...
## Comptel
f_Comptel = open('./Comptel_erg_MeV.dat', 'r')
...
## Egret
f_Egret = open('./EGRET_erg_Mev.dat', 'r')
...
Sens_value_1deg=Sens_value_1deg[5:]
Sens_value_5deg=Sens_value_5deg[5:]
Sens_value_90deg=Sens_value_90deg[5:]
energy_PLOT=energy_PLOT[5:]
...
plt.plot(energy_SPI,Sens_SPI,label='SPI Sensitivity 1yr ', linestyle='dashed')
plt.plot(energy_IBIS_ISG,Sens_IBIS_ISG,label='IBIS_ISGRI 1yr', linestyle='dashed')
plt.plot(energy_IBIS_PIC,Sens_IBIS_PIC,label='IBIS_PICsIT 1yr', linestyle='dashed')
plt.plot(energy_Comptel,Sens_Comptel,label='Comptel 9yr', linestyle='dashed')
plt.plot(energy_Egret,Sens_Egret,label='Egret 9yr', linestyle='dashed')

#ax1.plot(back_albedo_energy,Sens_value_erg,label='CrystalEye Sensitivity 1yr', linestyle='dashed')
ax1.plot(energy_PLOT,Sens_value_erg_1deg,label=r"CE Sensitivity 1yr - $\Delta \theta$= 1 deg", linestyle='solid')
ax1.plot(energy_PLOT,Sens_value_erg_5deg,label=r"CE Sensitivity 1yr - $\Delta \theta$= 5 deg", linestyle='solid')
ax1.plot(energy_PLOT,Sens_value_erg_90deg,label=r"CE Sensitivity 1yr - $\Delta \theta$= 90 deg (Full Sky)", linestyle='solid')
...
plt.savefig('Sensitivity_fromWeightedBack.png',dpi=200)
